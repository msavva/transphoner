 ...
 */
object SolrUtils {
  def query(solrServer: SolrServer, solrQuery: SolrQuery,
    ...
    def _queryHelper(buffered: Seq[SolrDocument], curStart: Int, count: Int, total: Long): Stream[SolrDocument] = {
      if (buffered.nonEmpty) {
        ...
        val responses = solrServer.query(solrQuery)
  ...
  def getFieldCount(url: String, fieldName: String, queryStr: String, filterQueries: Seq[String]): Map[String, Long] = {
    val server: HttpSolrServer = new HttpSolrServer(url)
    val map: Map[String, Long] = getFieldCount(server, fieldName, queryStr)
    server.shutdown
    map
  }

  def getFieldCount(server: SolrServer, fieldName: String): Map[String, Long] = {
    getFieldCount(server, fieldName, "*:*")
  }

  def getFieldCount(server: SolrServer, fieldName: String, queryStr: String): Map[String, Long] = {
    getFieldCount(server, fieldName, queryStr, null)
  }

  def getFieldCount(server: SolrServer, fieldName: String, queryStr: String, filterQueries: Seq[String]): Map[String, Long] = {
  ...
  def queryAndProcessSolrResults(server: SolrServer, query: SolrQuery, p: (SolrDocument,Int) => _,
                                 batchSize: Int = 1000,
