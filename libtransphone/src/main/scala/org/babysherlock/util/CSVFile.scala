...
class CSVFile(fileName: String, charset: String = "UTF-8",
  ...
  val fieldIndices = new collection.mutable.HashMap[String, Int]
...
//    m.foreach( k1v => k1v._2.foreach( k2v => println(k1v._1 + "," + k2v._1 + "," + k2v._2.size )))
  ...
  private def open() = {
    if (IOUtils.isWebFile(fileName)) {
      ...
      val bytes = WebUtils.loadBytes(fileName)
      new CSVReader(new InputStreamReader(IOUtils.inputStreamAutoDecompress(fileName, bytes.get), charset), separator, quote, escape)
    } else {
      new CSVReader(new InputStreamReader(IOUtils.inputStreamAutoDecompress(fileName), charset), separator, quote, escape)
    }
  }
...
object CSVFile {
  ...
  def filterCSV(id: String, values: Set[String], in: String, out: String) {
    ...
    val filteredRows = csv.rows.filter(row => {values.contains(row(id))})
